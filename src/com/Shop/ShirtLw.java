...
public class ShirtLw extends Shirts{
	ShirtLw() {
		super("White", "Large", 500);
	}
static Shirts sl=new ShirtLw();
static int q=10;
	 ...
	 System.out.println("Would you like to purchase?\n press 1 for yes and 2 for No");
	   ...
	   try {
		...
		if(options.equalsIgnoreCase("1"))
		{
			...
			{
				Tester.c1.price+=(sl.price*sl.quantity);
				//System.out.println("price into q "+(sms.price*sms.quantity)+" and T price "+Tester.c1.price);
				q-=sl.quantity;
				Bill.arr.add(new Bill("Large Shirts",sl.price,sl.quantity));
				break;
				}
